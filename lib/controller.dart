...
import 'package:jichanglianmeng/plugins/app.dart';
...
extension InitControllerExt on AppController {
  Future<void> _init() async {
    ...
    autoUpdateProfiles();
    autoCheckUpdate();
    unawaited(_ref.read(brandConfigProvider.notifier).refresh());
    ...
    await _initCore();
    await _initStatus();
    _openAirportWhenNoProfiles();
    _ref.read(initProvider.notifier).value = true;
  }

  void _openAirportWhenNoProfiles() {
    final hasProfiles = _ref.read(
      profilesProvider.select((state) => state.isNotEmpty),
    );
    if (!hasProfiles) {
      toPage(PageLabel.airport);
    }
  }
  ...
  Future<void> checkUpdateResultHandle({
    ...
    if (data != null) {
      ...
      if (res == true) {
        launchUrl(Uri.parse('https://github.com/$forkRepository/releases/latest'));
